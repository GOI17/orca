export function getStatusPluginFactorySource(options: { emitSessionStart: boolean }): string[] {
  return [
    ...
    '    try {',
    '      for await (const incoming of ctx.event.subscribe({ signal: controller.signal })) {',
    '        if (controller.signal.aborted) break;',
    '        // V2 subscriptions span the server, so retain this location only.',
    '        if (',
    '          incoming?.location &&',
    '          (incoming.location.directory !== ctx.location?.directory ||',
    '            incoming.location.workspaceID !== ctx.location?.workspaceID)',
    '        ) continue;',
    '        // V1 uses properties; V2 uses data.',
    '        const event = incoming?.properties || incoming?.data === undefined',
    '          ? incoming',
    '          : { ...incoming, properties: incoming.data };',
    '        await hooks.event({ event });',
    '      }',
    '    } catch (err) {',
    '      if (!controller.signal.aborted) {',
    '        console.warn("[orca-hook] OpenCode V2 event subscription failed:", err?.message || err);',
    '      }',
    '    }',
    '  })();',
    '',
    ...
    '}',
    '',
    '// V1 calls server; V2 calls setup.',
    'export default {',
