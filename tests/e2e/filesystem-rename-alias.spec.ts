...
test.describe('filesystem rename aliases', () => {
  test.skip(process.platform !== 'darwin', 'Requires native APFS alias behavior')

  test('renames one APFS entry through its Unicode alias', async ({ orcaPage, seededRepoPath }) => {
    const directory = seededRepoPath
    const suffix = Date.now().toString(36)
    const originalPath = path.join(directory, `rename-alias-${suffix}-straße.md`)
    const renamedPath = path.join(directory, `rename-alias-${suffix}-STRASSE.MD`)
  ...
  test('keeps dotless and ASCII I destinations distinct through IPC', async ({
    ...
    const directory = seededRepoPath
    const suffix = Date.now().toString(36)
    const firstPath = path.join(directory, `rename-dotless-first-${suffix}.md`)
    const secondPath = path.join(directory, `rename-dotless-second-${suffix}.md`)
    const dotlessDestination = path.join(directory, `rename-destination-${suffix}-ı.md`)
    const asciiDestination = path.join(directory, `rename-destination-${suffix}-I.md`)
